...
# MobileLISA: More lightweight, faster LISA model with extension on reasoning human-object interaction via LLMs

## Abstract
To be filled.

## New features
1. Support for 100DOH dataset. Download the original dataset from [100DOH](https://fouheylab.eecs.umich.edu/~dandans/projects/100DOH/agreement.html?pascal_voc_format)
which is VOC format, so it need to be converted to ADE20K format by running `utils/100doh_preprocess`. Details of the conversion process
can be found in the `utils/README.md`.
